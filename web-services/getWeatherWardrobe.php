...
if(!$res_select_user){
    ...
    if(mysql_num_rows($res_select_user)>0){
        ...
        $sql_select_images="Select * from ps_images where userId='".$row_user['id']."' and id in (Select imgId from ps_seasons where `season` = '".$_POST['season']."') or id in (Select imgId from ps_types where type in (Select event from ps_events where date = '".$dateT."'))";
